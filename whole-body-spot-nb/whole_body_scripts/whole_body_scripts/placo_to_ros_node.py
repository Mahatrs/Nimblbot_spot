...
class PlacoToROSNode(Node):
    ...
    def __init__(self, args):
        ...
        conf = load_robot_config("nb60_v7_3_4t+poignet")
    ...
    def compute_placo_positions(self):
        ...
        active_mask_joints = np.array(
            [True, False, True, False] * 12 + [True] * 1,
            dtype=bool
        )
...
def parse_args():
    ...
    parser.add_argument("--Xtarget", default=0.35, type=float)
    parser.add_argument("--Ytarget", default=0.05, type=float)
    parser.add_argument("--Ztarget", default=0.1, type=float)
