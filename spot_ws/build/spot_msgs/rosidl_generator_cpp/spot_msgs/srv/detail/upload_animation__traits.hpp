  ...
  std::ostream & out)
{
  ...
  {
    if (msg.animation_proto_serialized.size() == 0) {
      out << "animation_proto_serialized: []";
    } else {
      out << "animation_proto_serialized: [";
      size_t pending_items = msg.animation_proto_serialized.size();
      for (auto item : msg.animation_proto_serialized) {
