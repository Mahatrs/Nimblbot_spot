  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = spot_msgs::srv::SetLocomotion_Request;
    is_plain =
      (
      offsetof(DataType, locomotion_mode) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _SetLocomotion_Request__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_SetLocomotion_Request(full_bounded, is_plain, 0);
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = spot_msgs::srv::SetLocomotion_Response;
    is_plain =
      (
...
static size_t _SetLocomotion_Response__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_SetLocomotion_Response(full_bounded, is_plain, 0);
