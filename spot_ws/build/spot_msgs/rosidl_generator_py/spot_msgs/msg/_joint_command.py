...
class JointCommand(metaclass=Metaclass_JointCommand):
    ...
    _fields_and_field_types = {
        ...
        'k_q_p': 'sequence<double>',
        'k_qd_p': 'sequence<double>',
    }
    ...
    def __init__(self, **kwargs):
        ...
        self.k_q_p = array.array('d', kwargs.get('k_q_p', []))
        self.k_qd_p = array.array('d', kwargs.get('k_qd_p', []))
    ...
    def __eq__(self, other):
        ...
        if self.effort != other.effort:
            return False
        if self.k_q_p != other.k_q_p:
            return False
        if self.k_qd_p != other.k_qd_p:
            return False
        return True
    ...
    @k_q_p.setter
    def k_q_p(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'k_q_p' array.array() must have the type code of 'd'"
            self._k_q_p = value
            return
        if __debug__:
            from collections.abc import Sequence
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'k_q_p' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._k_q_p = array.array('d', value)

    @builtins.property
    def k_qd_p(self):
        """Message field 'k_qd_p'."""
        return self._k_qd_p

    @k_qd_p.setter
    def k_qd_p(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'k_qd_p' array.array() must have the type code of 'd'"
            self._k_qd_p = value
            return
        if __debug__:
            from collections.abc import Sequence
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'k_qd_p' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._k_qd_p = array.array('d', value)
