...
parser.add_argument('--Xtarget', default=1.8, type=float,
...
#arm = placo.RobotWrapper(MJCF_PATH_ARM
...
def update_arm_reach():
    ...
    arm_tcp_task.configure("arm_tcp_task", "soft", 10.0)
    arm_tcp_task.target_world = target
    arm_solver.solve(True)
    arm.update_kinematics()
    viz_arm.display(arm.state.q)
...
def loop():
    ...
    point_viz("target_bras", target, radius=0.05, color=0x00FF00)
