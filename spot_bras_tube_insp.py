...
def circle_target(theta, cylinder_center, radius):
    return cylinder_center + np.array([radius * np.cos(theta),0.0,radius * np.sin(theta)])
...
ARM_MJCF       = "/home/mtouri/Desktop/Maha_folder/robot_descriptions_maha/mjcf_description_tool/nb-55-v7_motor_tool.xml"
...
TARGET_START  = CYLINDER_CENTER + np.array([CYLINDER_RADIUS, 0.0, 0.0])
...
def loop():
    ...
    global traj_time, traj_time_2, traj_time_3
    ...
    # ----------------------------------------------------------
        ...
        if traj_time >= TRAJ_DURATION:
            ...
            _save_tcp_lock()          #on verouille le tcp pour reculer
            done        = False
            phase_time  = 0.0
            phase_index = 0
            mode        = MODE_RETREAT_X
            prepare_next_phase("x", spot_goal_xyz_retreat)

    # ----------------------------------------------------------
    # MODE_RETREAT_X
    # ----------------------------------------------------------
    elif mode == MODE_RETREAT_X:
        if not done:
            base_pos_task.mask.set_axises("yz")
            phase_time += dt
            tau = min(phase_time, T_TROT)
            ...
            cur    = robot.get_T_world_frame(BASE_FRAME)[:3, 3].copy()
            cur[0] = spot_goal_xyz_retreat[0]
            base_pos_task.target_world = cur
            base_pos_task.mask.set_axises("xyz")
            solver.solve(True)
            robot.update_kinematics()

            print(f"[RETREAT X done] X = {robot.get_T_world_frame(BASE_FRAME)[0,3]:.4f}")

            base_pos_task.mask.set_axises("yz")
            traj_time_2     = 0.0
            transition_time = 0.0
            mode            = MODE_RETREAT_PITCH

    # ----------------------------------------------------------
    # MODE_RETREAT_PITCH
    # ----------------------------------------------------------
    elif mode == MODE_RETREAT_PITCH:
        for foot in FEET:
            foot_tasks[foot].target_world = foot_targets_world[foot]

        transition_time += dt
        s = smooth(transition_time / RETREAT_PITCH_DURATION)

        base_ori_task.R_world_frame = (rot_z(spot_goal_yaw) @rot_y(spot_goal_pitch + RETREAT_PITCH_FORWARD * s) @rot_x(spot_goal_roll))
        base_pos_task.mask.set_axises("xyz")
        base_pos_task.target_world = robot.get_T_world_frame(BASE_FRAME)[:3, 3].copy()
        ...
        if transition_time >= RETREAT_PITCH_DURATION:
            print("[RETREAT PITCH done]")
            traj_time_2 = 0.0
            mode        = MODE_ARM_TRAJ_2

    # ----------------------------------------------------------
    # MODE_ARM_TRAJ_2 — partie 2 : theta pi/3 → pi
    # ----------------------------------------------------------
    elif mode == MODE_ARM_TRAJ_2:
        for foot in FEET:
            foot_tasks[foot].target_world = foot_targets_world[foot]
        ...
        traj_time_2 += dt
        s     = smooth(traj_time_2 / TRAJ_DURATION_2)
        theta = np.pi / 3.0 + (np.pi - np.pi / 3.0) * s

        target_now   = circle_target(theta, CYLINDER_CENTER, CYLINDER_RADIUS)
        R_target_now = tcp_rotation_tangent(theta, direction=-1.0)
        update_arm_reach(target_now, R_target_now)

        if traj_time_2 >= TRAJ_DURATION_2:
            print("[ARM_TRAJ_2 done]")
            done            = False
            phase_time      = 0.0
            phase_index     = 0
            transition_time = 0.0
    ...
    if mode == MODE_ARM_TRAJ:
        pass   
    elif mode == MODE_ARM_TRAJ_2:
        pass  
    elif mode in (MODE_RETREAT_X, MODE_RETREAT_PITCH):
        if tcp_locked_pos is not None and tcp_locked_R is not None:
            update_arm_hold()
        else:
            target_now   = circle_target(np.pi / 3.0, CYLINDER_CENTER, CYLINDER_RADIUS)
            R_target_now = tcp_rotation_tangent(np.pi / 3.0, direction=-1.0)
            update_arm_reach(target_now, R_target_now)
    else:
        if tcp_locked_pos is not None and tcp_locked_R is not None:
            update_arm_hold()    
    ...
    viz.viewer["my_part"].set_object(
        g.StlMeshGeometry.from_file("/home/mtouri/Downloads/tube_1.stl")
    )
    viz.viewer["my_part"].set_transform(
        tfm.concatenate_matrices(
            tfm.translation_matrix([1.5, 0.31, 0.535]),
            tfm.rotation_matrix(np.pi / 2, [1, 0, 0])
        )
    )
